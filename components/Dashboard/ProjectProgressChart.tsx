...
const ProjectProgressChart: React.FC<ProjectProgressChartProps> = ({ projects }) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case '施工中': return '#3b82f6'; // blue
      case '验收中': return '#a855f7'; // purple
      case '已完工': return '#10b981'; // green
        ...
        <div className="space-y-4">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 80, bottom: 5 }}>
              ...
              />
              <Bar dataKey="进度" radius={[0, 8, 8, 0]}>
                {chartData.map((entry, index) => {
                  const project = projects[index];
                  return (
                    <Cell key={`cell-${index}`} fill={getStatusColor(project.status)} />
                  );
                })}
              </Bar>
            </BarChart>
