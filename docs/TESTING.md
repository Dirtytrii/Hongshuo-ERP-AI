...
### Phase 5（壳层/消息中心/注册源）回归点

- **壳层接入**
  - `App.tsx` 必须使用 `AppShell`（sidebar/header/children slot），不应保留旧的内联 header/sidebar。
  - 侧栏必须来自注册源：`AppSidebar` 内部使用 `getVisibleSidebarSections(...)`（或等价逻辑），可根据权限与 adminOnly 正确显隐。

- **Header 标题与消息中心**
  - Header 标题应来自注册源：`getTabTitle(activeTab)`。
  - 消息中心红点/数量必须基于 `modules/dashboard/services/alertSummary.buildAlertSummary(...)` 产出的 `alerts` 聚合值。
  - 消息中心展开时应渲染 `MessageCenterPopover`；点击 overlay 能关闭 popover。
  - 点击某条 alert：应调用统一导航函数（至少 `setActiveTab(targetTab)`），并关闭 popover。

- **深链初始化与权限回落（P1）**
  - 支持 `?tab=projects&id=xx`：初始化时应跳到对应 tab，并设置项目 id（若格式非法则忽略 id）。
  - 若深链 tab/默认 tab 无权限访问，应回落到 `dashboard`（或 `DEFAULT_TAB` 的可访问替代），且不应卡死在“无权限”页。

- **待审批出库口径（P1）**
  - `pending-stock` 统计应与文案一致：仅统计 `stockLogs` 中 `status='pending'` 且 `type='out'` 的记录。
