...
## 14. 第十轮批量任务池：项目弹窗主线 + 驳回弹窗前置 + 角色类型清理

### 目标

第十轮继续多 agent 推进，但仍遵守“同一时间最多一个 agent 改 `App.tsx`”。本轮安排 3 个可并行任务：

- Agent L：主线任务，抽出项目编辑弹窗组件，会触碰 `App.tsx`。
- Agent M：前置任务，新建驳回原因弹窗组件和测试，不接入 `App.tsx`。
- Agent N：清理 `RoleManagement.tsx` 中低风险 `any` warning，不触碰 `App.tsx`。

### 当前依据

2026-05-20 复核结果：

- 第九轮 H/I/J 和后续 K 均已完成；当前 `InventoryItemModal` 已接入。
- `npm run lint` 通过，0 errors、45 warnings。
- `npm run test:run` 通过，19 个测试文件、90 个测试全部通过。
- `npm run build` 通过，仍有既有 Vite 大 chunk 提示。
- `App.tsx` 当前仍直接渲染 `isProjectModalOpen` 下的项目编辑弹窗。
- `App.tsx` 当前仍直接渲染 `isRejectNoteModalOpen` 下的驳回原因弹窗。
- `components/Users/RoleManagement.tsx` 当前有 2 个 `catch (e: any)` 低风险 warning。
...
- `components/Projects/ProjectFormModal.test.tsx`
- `docs/AGENT_WORK_PLAN.md`

#### 具体要求

1. 新建 `ProjectFormModal`。
   - 组件负责渲染遮罩、标题“新建项目/编辑项目”、项目名称、项目编号、项目经理、项目状态、合同金额、控制预算、只读已收款/进度/成本信息、开始/结束日期、取消和创建/更新按钮。
   - 继续复用 `components/ui/SearchableSelect`，不要改 `SearchableSelect` 本身。
   - 组件接收 `projectForm`、`editingProject`、`userOptions`、`onProjectFormChange`、`onClose`、`onSubmit`。

2. `App.tsx` 接入组件。
   - 删除 `App.tsx` 中 `isProjectModalOpen` 下的大段项目弹窗内联 JSX。
   - 保留 `isProjectModalOpen && <ProjectFormModal ... />` 的装配。
   - `App.tsx` 继续负责 `projectForm` / `setProjectForm`、`editingProject`、`userOptions`、`openProjectModal`、`handleSaveProject`、刷新、toast 和弹窗开关。
   - 不要把 `apiService.createProject`、`apiService.updateProject`、`reloadCoreData` 或用户选项加载移动进组件。

3. 保留现有行为。
   - 编辑项目时项目编号输入仍禁用。
   - 项目经理选项仍来自 `userOptions.map((u) => ({ value: u.username, label: u.username }))`。
   - 项目状态仍为“施工中 / 验收中 / 已完工”。
   - 控制预算空值仍写回 `''`，否则写回数字。
   - 已收款、进度、材料成本、人工成本、其他成本仍为只读展示，并保留原文案。
   - 取消和关闭按钮仍只关闭弹窗。
   - 提交按钮仍调用 `handleSaveProject`，文案仍为“创建/更新”。

4. 测试要求。
   - 新增 `components/Projects/ProjectFormModal.test.tsx`。
   - 至少覆盖：新建/编辑标题、编辑时项目编号禁用、项目经理选择回调、状态选择回调、控制预算空值和数字写回、只读财务/进度字段展示、取消/关闭回调、创建/更新提交回调。

#### 验收命令

- `npm run test:run -- components/Projects/ProjectFormModal.test.tsx`
- `npm run test:run`
- `npm run build`
- 建议 `npm run lint`，记录 warning 数量。
...
- `App.tsx` 不再保留项目编辑弹窗的大段 JSX。
- 项目弹窗用户可见文案、字段写回、禁用状态和提交行为不回退。
- 新组件测试覆盖关键渲染与回调。
- 及时提交，提交信息建议：`抽出项目编辑弹窗组件`。

### Agent M：新建驳回原因弹窗组件测试前置

#### 建议写入范围

- `components/ApprovalCenter/RejectNoteModal.tsx`
- `components/ApprovalCenter/RejectNoteModal.test.tsx`
- `docs/AGENT_WORK_PLAN.md`

#### 具体要求

- 只新建组件和测试，不接入 `App.tsx`，避免和 Agent L 冲突。
- 组件从当前 `App.tsx` 的 `isRejectNoteModalOpen` 内联驳回原因弹窗抽象而来。
- 组件负责渲染遮罩、标题“拒绝原因”、说明文字“请输入拒绝原因（可选）”、备注 textarea、取消按钮和确认拒绝按钮。
- 组件接收 `rejectNote`、`onRejectNoteChange`、`onCancel`、`onConfirm`。
- 取消/关闭由父组件负责清空 `pendingRejectLogId` 和 `rejectNote`；组件只调用 `onCancel`。
- 确认按钮只调用 `onConfirm`，不要在组件中访问 `window.__pendingRejectIsFinance`。
- 测试至少覆盖：标题/说明/placeholder 渲染、输入回调、取消/关闭回调、确认回调。
...
- 新组件和测试可独立通过。
- 不修改 `App.tsx`。
- 及时提交，提交信息建议：`补充驳回原因弹窗组件`

### Agent N：清理 RoleManagement 低风险 any

#### 建议写入范围

- `components/Users/RoleManagement.tsx`
- `docs/AGENT_WORK_PLAN.md`

#### 具体要求

- 只处理 `components/Users/RoleManagement.tsx` 中两个 `catch (e: any)` warning。
- 优先改为 `catch (e: unknown)`，并用 `e instanceof Error ? e.message : '...'` 保留原错误提示回退语义。
- 不改角色新增/编辑/删除业务流程。
- 不处理 `App.tsx` 或 `services/apiService.ts` 中的 `any` warning。
...
- `RoleManagement.tsx` 不再报告 `no-explicit-any` warning。
- `npm run lint` 仍为 0 errors，warning 数量预期从 45 降到约 43。
- 及时提交，提交信息建议：`清理角色管理错误类型`

### 并行边界

- Agent L 是本轮唯一允许修改 `App.tsx` 的任务。
- Agent M 不接入 `App.tsx`，只做组件和测试前置。
- Agent N 只改 `components/Users/RoleManagement.tsx`。
- 三个任务都完成后，再安排 Agent O 接入 `RejectNoteModal`。

## 15. 后续候选任务

第十轮批量任务完成并提交后，再安排以下任务，暂不同时开工：

1. 继续按页面域切分 `App.tsx`。
   - 优先接入 `RejectNoteModal`，再评估项目详情相关弹窗和剩余 AI 区域。
...
- 状态：第十轮主线，可立即开工；这是本轮唯一允许修改 `App.tsx` 的任务。
- 建议写入范围：`App.tsx`、`components/Projects/ProjectFormModal.tsx`、`components/Projects/ProjectFormModal.test.tsx`、`docs/AGENT_WORK_PLAN.md`。
- 目标：把 `App.tsx` 中 `isProjectModalOpen` 下的项目编辑弹窗抽出为 `ProjectFormModal`，保留项目表单状态、用户选项、提交、刷新和 toast 流程。
- 验收：`npm run test:run -- components/Projects/ProjectFormModal.test.tsx`、`npm run test:run`、`npm run build`，建议 `npm run lint`。
- 建议提交信息：`抽出项目编辑弹窗组件`

#### 任务 M：新建驳回原因弹窗组件测试前置

- 状态：第十轮并行任务，可立即开工；不允许修改 `App.tsx`。
- 建议写入范围：`components/ApprovalCenter/RejectNoteModal.tsx`、`components/ApprovalCenter/RejectNoteModal.test.tsx`、`docs/AGENT_WORK_PLAN.md`。
- 目标：先把驳回原因弹窗组件和测试准备好，给后续接入 `App.tsx` 铺路。
- 验收：`npm run test:run -- components/ApprovalCenter/RejectNoteModal.test.tsx`、`npm run test:run`、`npm run build`。
- 建议提交信息：`补充驳回原因弹窗组件`

#### 任务 N：清理 RoleManagement 低风险 any

- 状态：第十轮并行任务，可立即开工；只允许修改 `components/Users/RoleManagement.tsx` 和计划文档。
- 建议写入范围：`components/Users/RoleManagement.tsx`、`docs/AGENT_WORK_PLAN.md`。
- 目标：把两个 `catch (e: any)` 改为 `unknown` 安全处理，保留错误提示回退语义。
- 验收：`npm run lint` warning 数量预期从 45 降到约 43，且 `npm run test:run`、`npm run build` 通过。
- 建议提交信息：`清理角色管理错误类型`
...
Agent L：执行 docs/AGENT_WORK_PLAN.md 的“第十轮批量任务池：项目弹窗主线 + 驳回弹窗前置 + 角色类型清理”里的“Agent L：抽出项目编辑弹窗组件”。这是本轮唯一允许改 App.tsx 的任务。

Agent M：执行同一章节里的“Agent M：新建驳回原因弹窗组件测试前置”。只新建组件和测试，不接入 App.tsx。

Agent N：执行同一章节里的“Agent N：清理 RoleManagement 低风险 any”。只改 components/Users/RoleManagement.tsx 和计划文档。

三者完成后，再安排 Agent O 接入 RejectNoteModal。不要让 O 和 L 同时进行。
```
...
### 2026-05-20 总控安排：第十轮批量任务池

- 已确认第九轮 H/I/J 和后续 K 均已完成，当前分支领先远端 4 个提交。
- 已复核 `App.tsx` 中 `isProjectModalOpen` 下仍保留项目编辑弹窗内联 JSX，适合作为第十轮唯一触碰 `App.tsx` 的主线任务。
- 已复核 `App.tsx` 中 `isRejectNoteModalOpen` 下仍保留驳回原因弹窗内联 JSX；本轮只安排组件和测试前置，不接入 `App.tsx`，避免冲突。
- 已复跑 `npm run lint`，确认当前仍为 0 errors、45 warnings；`components/Users/RoleManagement.tsx` 有 2 个低风险 `catch (e: any)` warning，可独立并行清理。
...
```text
你现在接手 /Users/cloudjiang/Projects/personal/Hongshuo-ERP-AI 的第十轮批量任务。请先阅读 docs/AGENT_WORK_PLAN.md，重点执行其中“第十轮批量任务池：项目弹窗主线 + 驳回弹窗前置 + 角色类型清理”。

本轮可以同时开 3 个开发 agent，但必须遵守边界：
- Agent L 是唯一允许修改 App.tsx 的任务。
- Agent M 不接入 App.tsx，只新建驳回原因弹窗组件和测试。
- Agent N 只改 components/Users/RoleManagement.tsx 和 docs/AGENT_WORK_PLAN.md。
- 三者完成后，再安排 Agent O 接入 RejectNoteModal。不要让 O 和 L 同时进行。

Agent L：抽出项目编辑弹窗组件
目标：把 App.tsx 中 isProjectModalOpen 下的项目编辑弹窗 JSX 抽成 components/Projects/ProjectFormModal.tsx。App.tsx 继续负责 projectForm / setProjectForm、editingProject、userOptions、openProjectModal、handleSaveProject、刷新、toast 和弹窗开关。
建议范围：
- App.tsx
- components/Projects/ProjectFormModal.tsx
- components/Projects/ProjectFormModal.test.tsx
...
Agent M：新建驳回原因弹窗组件测试前置
目标：新建 components/ApprovalCenter/RejectNoteModal.tsx 和 components/ApprovalCenter/RejectNoteModal.test.tsx，但不要接入 App.tsx。
建议范围：
- components/ApprovalCenter/RejectNoteModal.tsx
- components/ApprovalCenter/RejectNoteModal.test.tsx
- docs/AGENT_WORK_PLAN.md
组件要求：
- 从当前 App.tsx 的 isRejectNoteModalOpen 内联驳回原因弹窗抽象而来。
- 渲染遮罩、标题“拒绝原因”、说明文字“请输入拒绝原因（可选）”、备注 textarea、取消按钮和确认拒绝按钮。
- 接收 rejectNote、onRejectNoteChange、onCancel、onConfirm。
- 取消/关闭只调用 onCancel，确认只调用 onConfirm。
- 不要在组件中访问 window.__pendingRejectIsFinance。
测试至少覆盖：
- 标题/说明/placeholder 渲染。
- 输入回调。
- 取消/关闭回调。
- 确认回调。
验收：
...
Agent N：清理 RoleManagement 低风险 any
目标：只处理 components/Users/RoleManagement.tsx 中两个 catch (e: any) warning，不处理其他文件。
建议范围：
- components/Users/RoleManagement.tsx
- docs/AGENT_WORK_PLAN.md
要求：
- 优先改为 catch (e: unknown)。
- 用 e instanceof Error ? e.message : '...' 保留原错误提示回退语义。
- 不改角色新增/编辑/删除业务流程。
- 不处理 App.tsx 或 services/apiService.ts 中的 any warning。
验收：
- npm run lint
