...
一个基于 React + Spring Boot 的现代化建筑行业 ERP 管理系统，集成 DeepSeek AI 智能分析能力，并处于 Phase 5 架构收口阶段。
...
- 🤖 **AI 决策室（可选）**
  - 集成 DeepSeek，用于对项目、库存、财务数据进行自然语言分析与策略建议
...
├── app/                          # 应用壳层：tab 注册源、导航、标题映射、消息中心、布局
├── modules/                      # 领域模块：仪表盘预警聚合、审批/财务域模板等 Phase 5 抽离逻辑
├── shared/                       # 共享页面骨架与复用 UI
├── components/                   # React 组件
│   ├── Dashboard/                # 仪表盘与图表
...
```env
# 推荐：OpenRouter Fans（OpenAI 兼容）
OPENROUTER_BASE_URL=https://openrouter.fans
OPENROUTER_API_KEY=sk-xxxxxxxx

# 兼容旧配置：仍可直接用 DeepSeek 官方 Key
# DEEPSEEK_API_KEY=your_deepseek_api_key_here
```

> 💡 **获取 API Key**: 在 `openrouter.fans` 控制台创建令牌（`sk-xxxx`），并写入 `.env.local` 后重启前端开发服务器。
...
# 数据初始化配置
# true: 每次启动都重置数据（当前仓库默认更适合演示/开发）
# false: 只在数据库为空时初始化（正式部署务必覆盖为 false）
app.data.reset-on-startup=true
...
AI 决策室当前接入 DeepSeek，对项目、库存、财务等经营数据提供分析与建议：
...
> 如果你不需要 AI 功能，可以不配置 `DEEPSEEK_API_KEY`，AI 决策室入口将隐藏或以降级模式运行。
...
| 阶段 | 状态 | 说明 |
| ---- | ---- | ---- |
| Phase 1 | 已完成 | 合同全生命周期、经营看板、预算执行看板、仪表盘穿透与运营风险区块均已可用。 |
| Phase 2 | 已完成 | 报销、借款、还款模块 CRUD、状态流和审批动作均已落地。 |
| Phase 3 | 已完成 | 部门主数据、部门成本归集、审批聚合与部门维度能力已可用。 |
| Phase 4 | 已完成 | 资料库自动归集、集成中心、移动端接口、审批中心聚合待办均已落地。 |
| Phase 5 | 进行中 | 正在做应用壳层、消息中心、预警聚合和共享页面骨架的结构收口。 |

> 本表为项目当前唯一进度矩阵。其他文档不再重复维护阶段状态，只引用此表。

## 🚀 五期开发启动（Phase 5）

已启动 Phase 5，当前迭代聚焦**消息中心能力沉淀 + 结构拆分**：

- 已将仪表盘预警聚合逻辑迁入 `modules/dashboard/services`，统一低库存、待审批、逾期项目、回款节点、里程碑超期、预算预警的聚合规则。
- 已将侧边栏导航和标题映射收口到 `app/navigation`，并开始将顶栏、消息中心、页面骨架从 `App.tsx` 中拆出。
- 当前目标是统一信息架构和页面壳层，不新增新的业务模块，不改动后端 API。
...
1. 检查 `.env` 或 `.env.local` 文件中的 `DEEPSEEK_API_KEY` 是否正确设置
2. 确认 API Key 是否有效
3. 检查网络连接是否正常
